...
public class PageRankCalculationReducer extends Reducer<Text, Text, Text, Text> {
    ...
    protected void readFile(URI file) throws IOException {
        ...
        while((line = br.readLine())!= null){
            ...
            if(page[1].equals("true")){
                ...
                pageRankIndex.put(lineDetails[0], page[0]);
            }
        }
        br.close();
    }
    ...
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        for (Text value : values) {
            ...
            {
                inlinks = value.toString().split(",");
                for (String link : inlinks) {
                    inlink = link.split(":")[0].trim();
                    if(!inlink.equals("-1"))  // -1 indicates dangling node index
                    {
                        prContribution = Double.parseDouble(link.split(":")[1]);
                        String inlinkPR = pageRankIndex.get(link.split(":")[0].trim());
        ...
        pageRank = (alpha/totalPageCount) + ((1- alpha)* (pr + (delta/totalPageCount)));
            ...
            {
                context.write(new Text(key.toString()), new Text( pageRank +"~~" +"false" ));
            }
            else
            {
                context.write(new Text(key.toString()), new Text( pageRank +"~~" +"true" ));
            }
